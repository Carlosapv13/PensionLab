...
| 1.1 | 2026-08-19 | Carlos Peraza (con asistencia de Claude, Anthropic, y revisión cruzada de Atlas — ChatGPT, OpenAI) | Agrega, en §5, la regla "Toda fecha usa el mismo patrón de captura" — estándar permanente de captura de fechas, surgido de la revisión final de S4-001 (Entregable 2, Sprint 4). El archivo `.docx` correspondiente todavía no se regeneró a partir de esta versión — pendiente, mismo criterio ya registrado para otros documentos de la Biblioteca de Conocimiento. |
...
| **Toda fecha usa el mismo patrón de captura** (Principio A) | PensionLab no pide información porque un formulario la contiene; la pide porque el expediente y el objetivo de la persona determinan que esa información es necesaria para responder su caso — y, cuando esa información es una fecha, la pide siempre con el mismo componente y el mismo comportamiento de interacción (tres campos día/mes/año, buffer de escritura del mes, aviso de día inválido para el mes/año elegidos), salvo que exista una razón funcional explícita y documentada para apartarse de él. Una persona que ya aprendió a responder una fecha en una pantalla no debería tener que reaprenderlo en la siguiente — cambiar de patrón sin razón sería, en la práctica, dejar de conversar y empezar a interrogar distinto cada vez. Las reglas de negocio de cada fecha (edad, fecha futura, orden entre un desde/hasta) permanecen deliberadamente fuera del componente compartido: cada pantalla sigue siendo responsable de las suyas — el componente resuelve captura e interacción, nunca negocio. | `CampoFechaDiaMesAnio.jsx`, consumido por "Datos iniciales" (fecha de nacimiento, S3-003, origen del patrón) y por la captura de historia de cotización RPM (S4-001, Entregable 2, Sprint 4) |
