...
| RPM-034 | Nivel completo auditable | Camino viable, cualquier ejercicio | Expandir "Ver el detalle auditable completo de este camino" | Los 7 `PasoAuditable` de Contrato F aparecen en orden (`DATOS_UTILIZADOS`, `IBL`, `TASA_REEMPLAZO`, `RESULTADO_MATEMATICO`, `AJUSTE_LEGAL`, `RESULTADO_FINAL`, `COMPARACION_OBJETIVO`), con etiquetas legibles, sin recalcular ni reinterpretar; caminos descartados nunca ofrecen el control | ✅ VALIDADO | Automatizado, incluida la ruta real completa | `nivelCompletoAuditable.helpers.test.js`; `DetallePasoAuditable.test.jsx`; `ProyectaTuPensionRPM.test.jsx` (describe `Nivel completo conectado`); `src/App.test.jsx` (`App — Caso A...regresión`, 7 pasos con cifras reales por la ruta real) | Validación visual real de Carlos/Atlas sobre capturas del Caso B, aprobada (2026-09-25) | MVP Oscar | E6.6/E6.7 (PL-260 §9.12-§9.13). Semanas fraccionarias se presentan como semanas completas + días restantes (`formatearSemanasComoTexto`), nunca con decimales crudos ni redondeadas hacia arriba. |
| RPM-035 | Confirmación de continuidad de cotización | Ejercicio RPM cualquiera | Ver Nivel esencial antes/después de pulsar "Entiendo y quiero explorar este escenario." | Antes: aviso "todavía no es una cifra confiable para publicar", cifra no publicable. Después: aviso desaparece cuando no hay otra causa de no publicable; el texto de vigencia se muestra tal cual (PL-260 §8.4) | ✅ VALIDADO | Automatizado, incluida la ruta real completa | `ConfirmacionContinuidadCotizacion.test.jsx`; `ProyectaTuPensionRPM.test.jsx`; `src/App.test.jsx` (Caso A: "cifras visibles antes de confirmar, aviso desaparece al confirmar") | Validación visual real de Carlos/Atlas, aprobada (2026-09-25) | MVP Oscar | E6.4/E6.5 (PL-260 §9.10-§9.11). |
| RPM-036 | Invalidación de la confirmación de continuidad | Confirmación ya dada | Cambiar edad objetivo, sexo, régimen actual o fecha de nacimiento; o cambiar objetivo económico/IBC/límite de esfuerzo/historia | Los cuatro primeros invalidan (vuelve a pedirse la confirmación); los cuatro segundos NUNCA invalidan | ✅ VALIDADO | Automatizado, incluida la ruta real completa (edad, sexo, fecha de nacimiento, objetivo económico) | `ProyectaTuPensionRPM.test.jsx` (describe `invalidación de la confirmación`); `src/App.test.jsx` (4 pruebas: edad/sexo/fecha invalidan por `App.jsx` real; objetivo económico no invalida) | Validación visual real de Carlos/Atlas, aprobada (2026-09-25) | MVP Oscar | Régimen actual: código de invalidación en `App.jsx` idéntico en forma a sexo/fecha (verificado por lectura), sin prueba equivalente por la ruta real de `App.jsx` — cambiarlo sin salir de `ProyectaTuPensionRPM.jsx` no es un flujo real (el único camino real, el botón "Editar tu régimen actual", ya está cubierto por `navegacionRPM.test.js`). |
| RPM-037 | Política jurídica NO_RESUELTA (Caso B) | `PoliticaAnclaIncrementoMujer` aplica y no se pudo verificar el texto resolutivo de la Sentencia C-197/2023 (mujer, semanas entre el mínimo de elegibilidad y 1.300) | Proyectar | Ejercicio `publicable:false`; sección "Políticas jurídicas de este ejercicio" con el nombre traducido y el mensaje jurídico íntegro; confirmar continuidad NUNCA resuelve la política (el aviso permanece) | ✅ VALIDADO | Automatizado, incluida la ruta real completa | `evaluarPoliticasEjercicioRPM.test.js`; `compararAnclaIncrementoRPM.test.js`; `PoliticasJuridicasInvolucradas.test.jsx`; `ProyectaTuPensionRPM.test.jsx` (describe `caso jurídico no publicable`); `src/App.test.jsx` (Caso B, 6 pruebas) | Validación visual real de Carlos/Atlas, aprobada (2026-09-25) | MVP Oscar | E5.4/E6.5/E7 (PL-260 §8, §9.11, §9.14). Nunca se elige una interpretación — la política permanece `NO_RESUELTA` en todos los casos verificados. |
| RPM-038 | No revelar cifra dependiente de una política jurídica no resuelta | Camino cuyo IBC futuro es la salida de una búsqueda hacia el objetivo (`aumentar-ibc-futuro`), bajo `PoliticaAnclaIncrementoMujer` `NO_RESUELTA` | Ver tarjeta resumida y Nivel completo de ese camino | IBC propuesto, aporte adicional, pensión proyectada, comparación con el objetivo, gráfico, `IBL` y `trazabilidadVentana` (incluido el período futuro anidado) quedan pendientes — ninguno expone su valor calculado; el camino `base` (independiente) conserva sus datos completos | ✅ VALIDADO | Automatizado, incluida la ruta real completa | `ProyectaTuPensionRPM.helpers.test.js` (`caminoIbcDependeDePoliticaJuridica`, `pasoUsaIbcEnDisputaDelCamino`); `ProyectaTuPensionRPM.test.jsx` (`RONDA 2 — regresión`); `src/App.test.jsx` (defectos 1-4, Caso B) | Validación visual real de Carlos/Atlas sobre las cifras exactas de la captura real (`$3.045.477`/`$2.891.245`/`$167.276`), aprobada (2026-09-25) | MVP Oscar | Hallazgo real de revisión visual (commit `f780db7`, dos rondas: la primera cubrió IBC/aporte/`DATOS_UTILIZADOS`; la segunda encontró que `IBL`/`trazabilidadVentana` del mismo camino seguían revelando la cifra indirectamente). Detalle completo en PL-260 §9.16. |
...
| RPM-NEG-008 | Política jurídica NO_RESUELTA | Mostrar, en cualquier parte de la pantalla (tarjeta, gráfico, orientación, límites, Nivel completo — incluidos pasos anidados como `trazabilidadVentana`), una cifra que dependa silenciosamente de haber elegido una interpretación jurídica no resuelta | Cualquier ejercicio con `PoliticaAnclaIncrementoMujer` `NO_RESUELTA` aplicable | Proyectar, expandir Nivel completo de cada camino | Toda cifra dependiente (IBC propuesto/aporte/pensión/comparación/gráfico, y para el camino cuyo IBC sale de una búsqueda hacia el objetivo, también `IBL`/`trazabilidadVentana`) aparece como "pendiente", nunca como valor calculado; ni el ejercicio ni ningún paso resuelve la política por su cuenta | Nunca una cifra silenciosamente dependiente de una interpretación no resuelta (PL-260 §0 punto 6) | 🚫 NO DEBE OCURRIR | Automatizado, incluida la ruta real completa | `ProyectaTuPensionRPM.test.jsx` (`E7`, `RONDA 2 — regresión`); `nivelCompletoAuditable.helpers.test.js`; `src/App.test.jsx` (defectos 1-4, Caso B) | Validación visual real de Carlos/Atlas sobre las cifras exactas de una captura real, aprobada (2026-09-25) | MVP Oscar | Hallazgo real, corregido en dos rondas (commit `f780db7`) — la primera ronda no cubría `IBL`/`trazabilidadVentana` del camino afectado. Ver PL-260 §9.16. |
| RPM-NEG-009 | Vocabulario interno | Mostrar un identificador interno de código (`continuidad_ibc_actual`, `busqueda_objetivo_rpm`, `VIDA_LABORAL_REQUIERE_HISTORIA_ESTRUCTURADA`, `PoliticaAnclaIncrementoMujer`, el campo `razonesIncompleto`, o equivalente) tal cual, sin traducir, en cualquier lugar visible | Cualquier ejercicio, cualquier camino | Proyectar, expandir Nivel completo | Todo código interno conocido se traduce a una frase legible (`ETIQUETAS_ORIGEN`, `ETIQUETAS_RAZON_VIDA_LABORAL_NO_EVALUADA`, `etiquetaNombrePolitica`); uno futuro sin traducción cae a un nombre humanizado, nunca al identificador crudo | Nunca un identificador interno de código visible para la persona usuaria | 🚫 NO DEBE OCURRIR | Automatizado, incluida la ruta real completa | `nivelCompletoAuditable.helpers.test.js`; `PoliticasJuridicasInvolucradas.test.jsx`; `src/App.test.jsx` (búsqueda literal en `document.body.textContent`) | Validación visual real de Carlos/Atlas, aprobada (2026-09-25) | MVP Oscar | Commit `f780db7` (PL-260 §9.16). |
...
Guía de prueba de producto — no técnica. Recorre estos 11 escenarios para
...
10. **Confirmación de continuidad y Nivel completo** (`RPM-034`/`RPM-035`) —
    en cualquier proyección, confirma "Entiendo y quiero explorar este
    escenario.": el aviso de "no confiable para publicar" debe desaparecer
