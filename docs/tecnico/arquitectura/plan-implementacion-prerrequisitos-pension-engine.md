...
- **Deuda arquitectónica registrada (Sprint 3, Slice RAIS — sin bloquear el Sprint actual)**: `LINEA_DE_TIEMPO_VIGENTE` en `data/legal/index.js` sigue siendo un arreglo hardcodeado de imports — agregar una norma nueva (ej. activar `reforma-2024.json` cuando la Corte falle sobre la Ley 2381) exige editar código, no solo agregar datos. Esto ya era el motivo original del Paso 2 (manifiesto de vigencia jurídica), nunca implementado. Al construir `data/assumptions/index.js` en el Slice RAIS se replicó deliberadamente "la misma mecánica" de `data/legal` (mismo criterio de mínima abstracción, Principio 9) — lo que significa que `LINEA_DE_TIEMPO_SUPUESTOS` heredó, sin resolverla, la misma limitación. Cuando se aborde el Paso 2, debe diseñarse para cubrir ambos casos, no solo `data/legal`.
- **Vacío arquitectónico registrado (Sprint 3, Slice RAIS — sin bloquear el Sprint actual)**: el sistema no tiene todavía una representación estructural para el caso en que existan dos interpretaciones jurídicas simultáneamente razonables, sin que ninguna sea claramente transitoria (distinto del caso ya cubierto por `estadoJuridico: 'transitorio'` + `permitirTransitorio`, que exige activar conscientemente un valor no firme, pero asume una sola interpretación en disputa, no varias coexistiendo). Ejemplo real ya presente en el código: `evidenciaIndiciosTransicion.js` reconoce en su propio comentario que la fecha de entrada en vigencia del sistema es distinta para servidores públicos territoriales, y hoy aplica solo la fecha general, documentándolo como limitación en vez de representarlo estructuralmente. No tiene solución propuesta todavía — queda como pregunta abierta de arquitectura.
